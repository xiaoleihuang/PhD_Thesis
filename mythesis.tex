...
First, we propose the \textit{temporality adaptation} that explicitly incorporates time into representation learning and modeling processes via feature augmentation and diachronic word embedding. The feature augmentation method aims to learn time-independent feature weights for document classifiers. We then develop an end-to-end time-adapted model with the diachronic word embedding under a time-driven framework.
Second, we propose the \textit{user factor adaptation} that models demographic factors and user interests using multitask learning framework.
To model demographic factors, document classifiers jointly predict the demographic factors and document category.
We further develop a multitask user embedding that jointly learns language, user behaviors and user interests.
We also examine and visualize impacts of temporality and user factor on word, topic, semantic and classifier levels.

Benefits of adapting demographic factors motivate us to examine if domain adaptation can reduce demographic biases. We release a multilingual hate speech corpus with author-level demographic labels. We examine demographic variations of user language and demographic biases of document classifiers. Following this, we develop a standard feature augmentation method to reduce demographic biases of the hate speech classifiers.
}
...
\acknowledgements{	%\OnePageChapter	% *MUST* BE ONLY ONE PAGE!
    ...
    The summer internship experience can not be so enjoyable without my friends including but not limit to: Anthony Michael Colas, Logan Lebanoff, Zhuowan Li, Naoto Inoue, Lixing Liu, Zheng Song and Jason Wu. My apologies for any omissions.
